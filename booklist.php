...
require('includes/config.php');
 session_start();
	...
	$cat=$_GET['subcatid'];
	$catname= $_GET['subcatnm'];
	
	$totalq="select count(*) \"total\" from books where subcat_name='$cat'";
	
	$totalres=oci_parse($con,$totalq) or die("Can't Execute Query...");
	
	$stmt=oci_execute($totalres);

	$totalrow=oci_fetch_assoc($totalres);
									...
									<div class="entry">
										...
										<table border="4" width="100%" >
											...
											<?php
												...
												//$query1="select * from books where sub_cat='$cat' LIMIT ".$k .",".$page_per_page;

												$query1="select * from books where subcat_name='$catname'";

												$stmt1 = oci_parse($con,$query1);
												
												oci_execute($stmt1) or die(" No Query");
												...
												while($row=oci_fetch_assoc($stmt1))
												{
													...
													echo '<td valign="top" width="20%" align="center">
														<a href="detail.php?id='.$row['BOOK_ID'].'&cat='.$_GET['subcatnm'].'">
